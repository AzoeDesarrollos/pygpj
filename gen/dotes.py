...
def elegir_dotes (dotes,espacios,clase):
    ...
    while True:
        if any(espacios.values()):
            ...
            if espacios['dt_rcl'] == True:
                print ('\n*'+t('Como aptitud racial, tienes una dote adicional para elegir.'))
                sublista[0][0] = s.DOTES
                sublista[0][1] = 'dt_rcl'
            if espacios['dt_nv'] == True:
                print('\n*'+t('En el ')+t(str(len(p.cla))+'º')+(' nivel, tienes una dote para elegir.'))
                sublista[1][0] = s.DOTES
                sublista[1][1] = 'dt_nv'
            if espacios['dt_cls'] == True:
                print('\n*'+t('Como aptitud de clase, tienes una dote adicional para elegir.'))
                sublista[2][0] = s.CLASES[clase]['dt_cls']
                sublista[2][1] = 'dt_cls'
        print (v.barra(p.CARS,s.alinis[p.alini],p.raza['Nombre']))
        print(t('Seleccione sus Dotes para este nivel'),end = '\n\n')
        print ('\n'+t('¿Que desea hacer?')+'\n')
        op = v.subselector(t('Opción'),opciones)
        if op == 0: # Elegir una nueva dote
            if not any(espacios.values()):
                print('\n'+t('No se pueden elegir más dotes por el momento'))
            else:
                posibles = GenerarListadeAyuda(AutoDot(s.DOTES,sublista[i][0],p.compW,p.conjuros,
                                                   s.ARMAS,s.HABS,s.ESCUELAS),s.DOTES)
                dote = SelDot(s.DOTES,posibles)
                if dote != '':
                    dotes.append(dote)
                    espacios[sublista[i][1]] = False
                if not any(espacios.values()):
...
def SelDot (DOTES, posibles):
    ...
    dote = ''
    while dote == '':
        dt = input(t('Dote')+': ').strip().capitalize()
        if dt == '':
            print (t('Escriba el nombre de una dote')+'\n')
        elif dt in (t('Volver'),t('Salir')):
            break
        elif dt not in nom:
            print (t('Por favor, escribe la dote correctamente')+'\n')
            ...
            if sub == 'w':
                ...
                dote = u_e (dt,dotes,s.ESCUELAS,nom,pre,des)
            elif sub == 'e?':
                dote = (dt,nom,pre,des)
            elif sub == 'c':
...
def info_dote (DOTES):
    ...
    while dote == '':
        dote = input('\nDote: ').capitalize()
        while dote not in dotes:
            print ('\nEscriba el nombre de la dote correctamente')
            dote = ''
